...
CROSS_COMPILER = docker run --rm -it -v $(GOPATH):/go -e GO111MODULE=on -w /go/src/github.com/uber/kraken golang:1.11.4 go build -o ./$@ ./$(dir $@); cp $(GOPATH)/src/$(PROJECT_ROOT)/$@ ./$(dir $@)
